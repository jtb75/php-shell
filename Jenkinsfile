node {
    ...
    stage ('Build Embedded Image') {
        container('build') {
            echo 'Building Embedded..'
            sh """
            chmod 777 /var/run/docker.sock
            docker build -t webapps/embedded-apache:$BUILD_NUMBER .
            """
        }
    }
    ...
    stage ('Push Embedded Image') {
        withCredentials([usernamePassword(credentialsId: 'harbor_cred', passwordVariable: 'HARBOR_PW', usernameVariable: 'HARBOR_USER')]) {
            container('build') {
                echo 'Pushing..'
                sh """
                docker tag webapps/embedded-apache:$BUILD_NUMBER 192.168.1.211:80/webapps/embedded-apache:$BUILD_NUMBER
                docker tag webapps/embedded-apache:$BUILD_NUMBER 192.168.1.211:80/webapps/embedded-apache:latest
                docker login --username ${HARBOR_USER} --password ${HARBOR_PW} 192.168.1.211:80
                docker push 192.168.1.211:80/webapps/embedded-apache:$BUILD_NUMBER
                docker push 192.168.1.211:80/webapps/embedded-apache:latest
                """
            }
        }
    }

    stage ('Cleanup Embedded') {
            container('build') {
            echo 'Pushing..'
            sh """
            docker rmi 192.168.1.211:80/webapps/embedded-apache:latest
            docker rmi 192.168.1.211:80/webapps/embedded-apache:$BUILD_NUMBER
            docker rmi webapps/embedded-apache:$BUILD_NUMBER
            """
        }
    }
